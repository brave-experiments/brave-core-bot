...
PR_COUNT=$(gh search prs --repo brave/brave-core --state open --sort updated --limit 1 --json number -- "updated:>=$CUTOFF_DATE" 2>/dev/null | python3 -c "import sys,json; print(len(json.load(sys.stdin)))" 2>/dev/null || echo "0")
